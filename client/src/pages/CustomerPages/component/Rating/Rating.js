...
function Rate({ feedbacks = [] }) {
    ...
    async function handleSubmit() {
        if (rated === 0) alert('Hãy đánh giá số sao');
            ...
            const newfeedback = { bookId: id, userId: user_id, rateStar: rated, review: valCmt };
            await axios.post('http://localhost:8080/api/feedback', newfeedback).then((response) => {
        ...
        <div className={cx('rating')}>
            <div className={cx('avg-rating')}>
                    ...
                    <div className={cx('numstar')}>
                        ...
                        <div className={cx('stars')}>
                            {avgStar ? (
                                ...
                                <Rating initialValue={0} readonly={true} />
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className={cx('filter')}>
                    <Button className={cx('filter-btn')} onClick={() => handleFilter(0)}>
                        Tất cả
                    </Button>
                    <Button className={cx('filter-btn')} onClick={() => handleFilter(5)}>
                        5 sao
                    </Button>
                    <Button className={cx('filter-btn')} onClick={() => handleFilter(4)}>
                        4 sao
                    </Button>
                    <Button className={cx('filter-btn')} onClick={() => handleFilter(3)}>
                        3 sao
                    </Button>
                    <Button className={cx('filter-btn')} onClick={() => handleFilter(2)}>
                        2 sao
                    </Button>
                    <Button className={cx('filter-btn')} onClick={() => handleFilter(1)}>
                        1 sao
                    </Button>
                </div>
            </div>
            ...
            {cmt ? (
                <div className={cx('feedback-field')}>
                    <img src={images.avatar} alt="" />
                    <div className={cx('info-review')}>
                        <div className={cx('info')}>
                            <p>{user_name}</p>
